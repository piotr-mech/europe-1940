...
| `resolveBattle(state, armyId, targetFieldId, rngSeed)` (`src/lib/battle.ts`) | function | Pure battle resolution (FR-007/008/009): modifier-stacked strengths ±20% seeded rolls, loser destroyed, clamped winner losses, city capture + queue cancel; returns new state + report + advanced seed; throws on illegal battles | `gameReducer` (`attackArmy`), tests |
