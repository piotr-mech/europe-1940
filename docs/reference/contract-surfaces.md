...
| `movementAllowance(state, army)` (`src/lib/supply.ts`) | function | Movement allowance for a turn: full `armySpeed` when supplied, max 1 when cut off (FR-011); the single source of truth for the cap | `gameReducer` (`endTurn` reset), S-06 (AI movement) |
