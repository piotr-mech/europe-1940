...
| 4 | Save/resume breaks as the state schema evolves — the two most-changed files are exactly where game state and its types live, and save validation is a pragmatic type-guard (catches truncation/version mismatch, not deep corruption), so a drifted save can load as a subtly broken game; or a storage failure is swallowed so the player only discovers the campaign was never saved when a refresh loses it (m3l5 swallowed-error sweep) | High | Medium | PRD FR-014 + Guardrails; hot-spot dirs `src/lib/`, `src/types.ts` (11 touches/30d); archive save-resume plan; m3l5 change autosave-failure-surfacing |
...
| #4 | save→load→save round-trips deep-equal; a save from any schema epoch either loads correctly or is cleanly discarded (never plays on as a subtly broken state); a finished game is never resurrected; storage failure degrades to no-persistence without crashing AND is surfaced — `saveGame` reports `"failed"`, the UI raises a banner, never silent | "the type-guard accepted it, so the state is sane" (it only catches truncation/version); "the game kept playing, so the save worked" (the swallowed-failure trap m3l5 closed) | full persisted-state shape; version envelope; when saves are cleared; which storage calls can throw | unit + integration | round-trip happy path only |
...
| e2e (browser) | Playwright (@playwright/test) | 1.63.0 | added m3l4: `npm run test:e2e`, config with `webServer` on `npm run dev`; five specs — seed (risk #4 reload persistence), input-blocked AI replay (risk #6 rendered UI), autosave-failure banner (risk #4 silent save loss, m3l5), exit-to-menu (risk #4 non-destructive exit, m3l5), delete-save (risk #4 user-initiated save deletion, change `delete-save`); no storageState (no auth, PRD) |
...
| e2e (`npm run test:e2e`) | CI on `main` (wired since m3l4) | required | cross-boundary browser risks (#4 reload persistence, #4 silent autosave failure, #6 rendered input blocking) |
